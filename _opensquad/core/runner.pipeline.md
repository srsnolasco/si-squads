...
5b. Run ID Slug Enrichment (if run_id_slug: true — see below)
6. Dashboard Handoff (to next step)
```

### Run ID Slug Enrichment

When a step's frontmatter contains `run_id_slug: true`, execute this after step output validation (step 4) and before dashboard handoff (step 6):

1. **Check if already enriched**: if current `run_id` already contains a slug (i.e., has more than 2 hyphens after the date portion, e.g. `2026-05-13-201921-some-slug`), skip — slug was already set.
2. **Extract title from output**: read the step's output file (after Output Path Transformation). Take the first `# ` markdown heading found in the file.
   - Example: `# Análise do Vídeo: Definição de posse` → title = `Definição de posse`
   - Strip any prefix before `:` if the heading contains `: `
3. **Generate slug** via bash:
   ```bash
   python3 -c "
   ...
   slug = re.sub(r'[^a-z0-9]+', '-', ascii_str.lower()).strip('-')[:40]
   print(slug)
   "
   ```
4. **Rename the run folder**:
   ```bash
   mv squads/{name}/output/{run_id}/ squads/{name}/output/{run_id}-{slug}/
   ```
5. **Update run_id** in working memory: `new_run_id = {run_id}-{slug}`
6. From this point on, ALL subsequent output paths use `new_run_id`. Update any in-memory references.
